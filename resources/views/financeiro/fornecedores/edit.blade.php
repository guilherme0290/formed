    ...
    <div class="w-full px-3 md:px-5 py-4 md:py-5 space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <h1 class="text-3xl font-semibold text-slate-900">Editar fornecedor</h1>
                <p class="text-sm text-slate-500">{{ $fornecedor->razao_social }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('financeiro.fornecedores.update', $fornecedor) }}" class="bg-white rounded-3xl border border-slate-100 shadow-sm p-5 space-y-5">
            @csrf
            @method('PUT')
            @include('financeiro.fornecedores._form', ['fornecedor' => $fornecedor])

            <div class="flex justify-end">
                <button class="px-4 py-2 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700">Atualizar fornecedor</button>
            </div>
        </form>
    </div>
@endsection
