        ...
        <div class="bg-white rounded-2xl shadow border p-6 space-y-4">
            ...
            <form id="acessoForm" method="POST" action="{{ route($routePrefix.'.acesso', $cliente) }}" class="space-y-4">
                ...
                <div class="grid sm:grid-cols-2 gap-2">
                    <button type="submit" @if(!$canManageAcesso) disabled title="Usu�rio sem permiss�o" @endif class="px-4 py-2 rounded-xl text-sm font-semibold w-full {{ $canManageAcesso ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-slate-200 text-slate-500 cursor-not-allowed' }}">
                        Criar acesso
                    </button>
