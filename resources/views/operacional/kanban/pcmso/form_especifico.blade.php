            ...
            <form method="POST"
                ...
                <div id="pcmso-tab-dados" class="space-y-4">
                    ...
                    <div class="rounded-xl bg-purple-50 border border-purple-100 px-4 py-3 text-xs text-purple-800 space-y-1">
                        ...
                        @if(isset($valorPcmsoEspecifico) && $valorPcmsoEspecifico !== null)
                            <p>Valor contratado do item: <strong>R$ {{ number_format((float) $valorPcmsoEspecifico, 2, ',', '.') }}</strong>.</p>
                        @endif
                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const duplicateWarning = @json(!$isEdit && $duplicidadeAtiva
                    ? "Ja existe um PCMSO Especifico pendente para este cliente. Deseja criar outro mesmo assim?"
                    : null);
